...
class PrivBayesNP(PrivBayes):
    """
    PrivBayes class without privacy, only epsilon used to calculated max parent size
    """

    def __init__(self, epsilon, theta_usefulness=4, network_init=None, root=None, verbose=True):
        super().__init__(epsilon=np.inf, theta_usefulness=theta_usefulness, epsilon_split=0, network_init=network_init, verbose=verbose)
        self.e2 = epsilon
        self.root = root
    ...
    def _max_domain_size(self, data, node):
        """Computes the maximum domain size a node can have to satisfy theta-usefulness"""
        node_cardinality = utils.cardinality(data[node])
        max_domain_size = self.n_records_fit_ * self.e2 / \
                          (2 * len(self.columns_) * self.theta_usefulness * node_cardinality)
        return max_domain_size
    ...
    def _init_network(self, X):
        ...
        # If root node is not set by user, randomly pick one
        if not self.root:
            root = np.random.choice(tuple(nodes))
        else:
            root = self.root
    ...
    def _compute_scaling_factor(self, node_parent_pairs):
        ...
        for idx, pair in enumerate(node_parent_pairs):
            ...
            scaling_factors[idx] = self._n_nodes_dp_computed * sensitivity / (self.epsilon * (1 - self.epsilon_split))
        return scaling_factors
